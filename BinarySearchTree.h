...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    int num = 0;
    if (node) {
      num += size_impl(node->left);
      num += size_impl(node->right);
      ++num;
    }
    return num;
  }
  ...
  static int height_impl(const Node *node) {
    int number = 0;
    if (node) {
      int nleft = height_impl(node->left);
      int nright = height_impl(node->right);
      number = 1 + (nleft > nright ? nleft : nright);
    }
    return number;
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    Node* lmax = max_element_impl(node->left);
    if (lmax != nullptr && !less(lmax->datum, node->datum))
      return false;
    Node* rmin = min_element_impl(node->right);
    if (rmin != nullptr && !less(node->datum, rmin->datum))
      return false;
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if (less(val, node->datum)) {
      Node* p = min_greater_than_impl(node->left, val, less);
      return (p != nullptr) ? p : node;
    }
